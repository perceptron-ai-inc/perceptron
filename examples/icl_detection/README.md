...
Need pixel-space coordinates for drawing or metrics? Call the built-in scaler once you know the rendered size:

```python
from PIL import Image

img = Image.open("examples/icl_detection/input.png")
pixel_boxes = res.points_to_pixels(*img.size)
...
The preview PNGs were generated by scaling the normalized coordinates into pixel space via `scale_points_to_pixels`:

```python
from pathlib import Path
from PIL import Image, ImageDraw
from perceptron import bbox, scale_points_to_pixels

BOXES = {
    'classA': ('classA.jpg', bbox(316, 136, 703, 906, mention='classA')),
    'classB': ('classB.webp', bbox(161, 48, 666, 980, mention='classB')),
    'input': ('input.png', bbox(38, 91, 934, 962, mention='classB')),
}

root = Path('examples/icl_detection')
for label, (filename, normalized_box) in BOXES.items():
    img = Image.open(root / filename).convert('RGB')
    w, h = img.size
    box = scale_points_to_pixels([normalized_box], width=w, height=h)[0]
    draw = ImageDraw.Draw(img)
    for offset in range(3):
        draw.rectangle(
            (
                box.top_left.x - offset,
                box.top_left.y - offset,
                box.bottom_right.x + offset,
                box.bottom_right.y + offset,
            ),
            outline='red',
        )
